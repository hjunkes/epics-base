...
/* char rtemsInit_NTP_server_ip[16] = "10.0.0.100"; */
...
// hpj comment ? this was a stopgap solution DEFVAR("new_filename", bootp_cmdline_init),
#undef DEFVAR
        {NULL}
    };
    (void)hook;
...
default_network_dhcpcd(void)
{
    ...
    assert( rv == 0 );
        ...
        if(prim_ifname)
          printf(" Primary interface name : %s\n", prim_ifname);
        else
          printf("NO Primary interface name\n");

    rtems_task_wake_after(RTEMS_MILLISECONDS_TO_TICKS(2000));

    unsigned idx = if_nametoindex("cpsw0");
    printk("cpsw0 interface index: %u (%s)\n", idx, idx ? "EXISTS" : "NOT FOUND");
